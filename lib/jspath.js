...
var translate = (function() {
    ...
    var body, vars, lastVarId, lastLabelId, unusedVars;
                        ...
                        body.push(
                            'for(', j, ' in ', curCtx, ') {',
                                'if(', curCtx, '.hasOwnProperty(', j, ')) {',
                                    v, '=', curCtx, '[', j, '];',
                                    generateAppendToArray(result, v),
                                '}',
                            '}') :
                        body.push(
                            v, '=', curCtx, '[', propStr, '];',
    ...
    function translateEqualityExpression(expr, dest, ctx) {
        ...
        var isRightArgLiteral = expr.args[1].type === SYNTAX.LITERAL;
        ...
        isRightArgLiteral?
            body.push('false;') :
            body.push('Array.isArray(', val2, ');');

        body.push('if(', isVal1Array, '&&', val1, '.length === 1) {',
                val1, '=', val1, '[0];',
                isVal1Array, '= false;',
            '}');
            ...
            body.push(
                'if(', isVal2Array, ') {',
                    ...
                    'while(', i, '<', len1, ') {',
                        j, '= 0;',
                        'while(', j, '<', len2, ') {');
                            writeCondition(expr.op, [val1, '[', i, ']'].join(''), [val2, '[', j, '++]'].join(''), tmpVal1, tmpVal2);
                            body.push(
                                dest, '= true;',
                                'break ', loop, ';',
                            '}',
        ...
        body.push(
                    'while(', i, '<', len1, ') {');
                        writeCondition(expr.op, [val1, '[', i, '++]'].join(''), val2, tmpVal1, tmpVal2);
                        body.push(
                            dest, '= true;',
                            'break;',
                        '}',
        ...
        if(!isRightArgLiteral) {
            ...
            'else if(', isVal2Array,') {',
                len2, '=', val2, '.length;',
                'while(', i, '<', len2, ') {');
                    writeCondition(expr.op, val1, [val2, '[', i, '++]'].join(''), tmpVal1, tmpVal2);
                body.push(
                        dest, '= true;',
                        'break;',
                    '}',
    ...
    function writeCondition(op, val1Expr, val2Expr, tmpVal1, tmpVal2) {
        complexBinaryOperators[op]?
            body.push(
                tmpVal1, '=', val1Expr, ';',
                tmpVal2, '=', val2Expr, ';',
                'if(', formatBinaryOperator(op, tmpVal1, tmpVal2), ') {') :
            body.push('if(', formatBinaryOperator(op, val1Expr, val2Expr), ') {');
    }
    ...
    function convertToBool(varName) {
        return ['(typeof ', varName, '=== "boolean"?', varName, ':',
            'Array.isArray(', varName, ')?', varName, '.length : !!', varName, ')'].join('');
    }
    ...
    function formatBinaryOperator(operator, val1, val2) {
        return binaryOperators[operator](val1, val2);
    }
    ...
    var binaryOperators = {
            ...
            '$==' : function(val1, val2) {
                return 'typeof ' + val1 + '=== "string" && typeof ' + val2 + '=== "string" &&' +
                    val1 + '.lastIndexOf(' + val2 + ') ===' + val1 + '.length - ' + val2 + '.length';
            },
            ...
            '$=' : function(val1, val2) {
                return '(function(v1, v2) {' +
                    'if(v1 === null || v2 === null) {' +
                        'return false;' +
                    '}' +
                    'var v1s = v1.toString().toLowerCase(),' +
                        'v2s = v2.toString().toLowerCase();' +
                    'return v1s.lastIndexOf(v2s) === v1s.length - v2s.length;' +
                '})(' + val1 + ', ' + val2 + ')';
            },
