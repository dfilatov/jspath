...
var parse = (function() {
    ...
    function parseProp() {
        ...
        if(type === TOKEN.ID || type === TOKEN.STR || (type === TOKEN.PUNCT && token.val === '*')) {
            return token.val;
        }
...
var translate = (function() {
    ...
    function translatePath(path, dest, ctx, isTopLevel) {
        ...
        while(i < len) {
            ...
            switch(item.type) {
                case SYNTAX.SELECTOR:
                    if(item.selector === '..') {
                        isResArray && body.push(isResArray, '= true;');
                        translateDescendantSelector(item, resVar, resVar);
                    }
                    else {
                        translateSelector(item, resVar, resVar, isResArray);
                    }
                    break;

                case SYNTAX.OBJ_PRED:
                    translateObjectPredicate(item, resVar, resVar);
                    break;

                case SYNTAX.POS_PRED:
                    translateArrayPredicate(item, resVar, resVar, isResArray);
                    break;
            }
        ...
        body.push(dest, '=');
        isResArray?
            body.push(isResArray, '?', resVar, ':', resVar, '[0]', ';') :
            body.push(resVar, ';');

        releaseVars(resVar);
    }

    function translateSelector(sel, dest, ctx, isResArray) {
        if(!sel.prop) {
            body.push(dest, '=', ctx, ';');
        }
        else {
            ...
            if(sel.prop === '*') {
                ...
                body.push(
                        val, '=', curCtx, '[', propStr, '];');
                        inlineAppendToArray(res, val, tmpArr, len, isResArray);
            }
            body.push(
                    '}',
                '}',
    ...
    function translateArrayPredicate(item, dest, ctx, isResArray) {
    ...
    function inlineAppendToArray(res, val, tmpArr, len, isResArray) {
        body.push(
            'if(', val, '!= null) {',
                'if(Array.isArray(', val, ')) {');
        isResArray && body.push(isResArray, '= true;');
        tmpArr && body.push(
                    len, '> 1?', tmpArr, '.push(', val, ') :');
        body.push(
                    res, '=', res, '.concat(', val,');',
                '}',
                'else {');
...
exports.params = function(_params) {
    if(!arguments.length) {
        return params;
    }
