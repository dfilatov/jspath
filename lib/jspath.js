...
function parseObjectPredicate() {
    ...
    return function(ctx, root, substs) {
        var res = [], i = 0, len = ctx.length, curCtx, exprRes;

        while(i < len) {
            exprRes = expr(curCtx = ctx[i++], root, substs);
            (typeof exprRes === 'boolean'? exprRes : toBoolean(exprRes)) && res.push(curCtx);
        }
