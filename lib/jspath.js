...
var translate = (function() {
        ...
        while(i < len) {
            ...
            switch(item.type) {
                ...
                case SYNTAX.POS_PRED:
                    if(translatePosPredicate(item, resVar, resVar) === false && isTopLevel && i === len) {
                        isResArray = false;
                    }
                    break;

                case SYNTAX.CONCAT_EXPR:
                    translateConcatExpr(item, resVar, resVar);
                    break;
            }
        }

        body.push(dest, '=', resVar, isResArray? ';' : '[0];');
        releaseVars(resVar);
    }
    ...
    function translateObjectPredicate(expr, dest, ctx) {
        ...
        body.push(
            ...
            'while(', i, '<', len, ') {',
                currentItem, '=', ctx, '[', i, '];');
                translateExpr(expr.arg, cond, currentItem);
        body.push(
        ...
        switch(expr.type) {
            ...
            case SYNTAX.BINARY_EXPR:
                arithmeticOperators[expr.op]?
                    translateArithmeticExpr(expr, dest, ctx) :
                    translateEqualityExpr(expr, dest, ctx);
                break;
    ...
    function translateEqualityExpr(expr, dest, ctx) {
    ...
    function translateArithmeticExpr(expr, dest, ctx) {
        var val1 = acquireVar(),
            val2 = acquireVar();

        translateExpr(expr.args[0], val1, ctx);
        translateExpr(expr.args[1], val2, ctx);
    ...
    function translateUnaryExpr(expr, dest, ctx) {
        ...
        translateExpr(expr.arg, val, ctx);
